...
//data = data.replaceAll('green', 0x00FF00);
//data = data.replaceAll('red', 0xFF0000);
//data = data.replaceAll('blue', 0x0000FF);

data = data.split('\n').map(el0 => el0.split(': '));
data = data.map(el0 => el0[1].split('; ')); // game == index + 1
data = data.map(el0 => el0.map(el1 => el1.split(', ')));
data = data.map(el0 => el0.map(el1 => el1.map(el2 => el2.split(' '))));

for(let i = 0; i < data.length; i++) // for each game
{
	let data2 = data[i];
	out: for (let j = 0; j < data2.length; j++) // for each group of set
	{
		let data3 = data2[j]
		for (let k = 0; k < data3.length; k++) // for each set
		{
			let data4 = data3[k];
			if (
				Number(data4[0]) > 12 && data4[1] === 'red' ||
				Number(data4[0]) > 13 && data4[1] === 'green' ||
				Number(data4[0]) > 14 && data4[1] === 'blue'
				)
			{
				result -= i+1;
				break out;
...
data = `Game 1: 1 green, 6 red, 4 blue; 2 blue, 6 green, 7 red; 3 red, 4 blue, 6 green; 3 green; 3 blue, 2 green, 1 red
