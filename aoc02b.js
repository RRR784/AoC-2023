...
for(let i = 0; i < data.length; i++) // for each game
{
	...
	//debugger
	let data2 = data[i];
	for (let j = 0; j < data2.length; j++) // for each group of set
	{
		let data3 = data2[j]
		for (let k = 0; k < data3.length; k++) // for each set
		{
			let data4 = data3[k];
			if (Number(data4[0]) > min_red && data4[1] === 'red')
			{
				min_red = Number(data4[0])
			}
			if (Number(data4[0]) > min_green && data4[1] === 'green')
			{
				min_green = Number(data4[0])
			}
			if (Number(data4[0]) > min_blue && data4[1] === 'blue')
			{
				min_blue = Number(data4[0])
			}
		}
	}
	debugger;
