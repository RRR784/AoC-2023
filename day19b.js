let go = () => {
	...
	data = data.split('\n\n');
	let data_a = data[0].split('\n').map(el=>el.replaceAll("{", ",").replaceAll("}", ""))

	// instructions
	data_a = data_a.map(el=>el.split(","));
	let instruction_a = {};
	data_a.forEach(el=>instruction_a[el[0]]=el.slice(1).map(el2=>el2.split(":")))

	let oe = Object.values(instruction_a)
	for(let i = 0; i < oe.length; i++)
	{
		let j = 0
		for(; j < oe[i].length-1; j++)
		{
			if(oe[i][j][0][1] == "<")
			 	(oe[i][j][0] = oe[i][j][0].split("<")).push("<");
			if(oe[i][j][0][1] == ">")
				(oe[i][j][0] = oe[i][j][0].split(">")).push(">")
			oe[i][j][0][1] = Number(oe[i][j][0][1])
		}
		oe[i][j] = oe[i][j][0]
	}


	console.dir(instruction_a, {depth: 10});

	let tree = {};
	let results = [];

	let proc = (part, instruction, current) => {
		let in_wf = instruction_a[instruction]
		current[instruction] = {};
		out: while(1)
		{
			let i = 0
			for(; i < in_wf.length - 1; i++)
			{

				current[instruction][i] = {};

				let id = in_wf[i][0][0] 	// 's'
				let test = in_wf[i][0][2]; 	// '<'
				let value = in_wf[i][0][1] 	// 1351
				...
				// then you modifiy fail
				// then you modifiy part
				if (test == ">")
				{
					fail[id][0] = fail[id][0]
					fail[id][1] = Math.min(fail[id][1], value)

					part[id][0] = Math.max(part[id][0], value+1)
					part[id][1] = part[id][1]

				}
				else if (test == "<")
				{
					fail[id][0] = Math.max(fail[id][0], value)
					fail[id][1] = fail[id][1]

					part[id][0] = part[id][0]
					part[id][1] = Math.min(part[id][1], value-1)
				}
				
				// handle pass
				let pointer = in_wf[i][1]
				if (pointer == 'R' || pointer == 'A')
				{
					current[instruction][i][pointer] = part;
					if (pointer == 'A')
						results.push(part);
				}
				else
				{
					proc(part, pointer, current[instruction][i]);
				}
			...
			if (pointer == 'R' || pointer == 'A')
			{
				...
				current[instruction][pointer].part = part;
				return;
			}
			else
			{
				current[instruction][pointer] = {};
				proc(part, pointer, current[instruction][pointer]);
				return;
			}
	...
	{
		sum += 
		(results[i].x[1] - results[i].x[0] + 1) * 
		(results[i].m[1] - results[i].m[0] + 1) * 
		(results[i].a[1] - results[i].a[0] + 1) * 
		(results[i].s[1] - results[i].s[0] + 1);
	}
	console.log(sum)
}

let data0 = `px{a<2006:qkq,m>2090:A,rfg}
pv{a>1716:R,A}
lnx{m>1548:A,A}
