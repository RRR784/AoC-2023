...
let go = () => 
{
	...
	console.log(data[120][110]);
	let current = {x: 110, y: 120};
	...
	let steps1;
	{
		...
		let step = 0;
		steps0 = [['S', step++], [data[y][x], step++]];
		while(1)
		{
			let pipe = data[y][x];
			direction = fromto(pipe, direction)
			x += direction.x;
			y += direction.y;
			if (x < 0 || x >= data[0].length || y < 0 || y >= data.length)
				throw new Error("Out of bounds pipe");
			let next_pipe = data[y][x]
			steps0.push([next_pipe, step])
			step++;
			if (next_pipe == "S")
				break;
		}
	...
	console.log(steps0)
	let steps2 = [];
	for(let i = 0, j = steps0.length - 1; i < steps0.length; i++, j--)
	{
		steps2.push(Math.min(steps1[i][1], steps0[j][1]));
	}
	console.log(steps2)
...
let data = `FF--7-|7F--7F--|7F77-.FJF7F-7-F|7.77.FF7-F7---7-FJ7.|J--FJJ-FF-.JJ7LF7.J7F-7-F77L7.7-7.7FF77FF7..F--L7JF--.FF.7F|JFL7.7FL77.-7|7.F7-7FJ.FJ.-
