	...
	for (let y = 0; y < h; y++)
	{
		...
		for (let x = 0; x < w; x++)
		{
			if (data[y][x] == '*')
			{
				...
				for (let j = y0; j <= y1; j++)
				{
					for(let i = 0; i < ranges[j].length; i++)
					{
						let lo = ranges[j][i][0];
						let hi = ranges[j][i][1];
						if ((x+1) >= lo && (x-1) < hi)
						{
							result_temp.push(nums[j][i]);
						}
					}
				}
				if (result_temp.length == 2)
				{
					results.push(result_temp);
				}
			}
		}
	}

	results = results.map(el => el[0] * el[1])
	results = results.reduce((a, c) => a + c);
