let go = () => {
	...
	// parts
	data_b = data_b.map(el=>el.split(",").map(el2=>el2.split("=")))
	let objects_b = [];
	data_b.forEach(el=>{
		let obj = {
			[el[0][0]] : Number(el[0][1]),
			[el[1][0]] : Number(el[1][1]),
			[el[2][0]] : Number(el[2][1]),
			[el[3][0]] : Number(el[3][1])
		}
		objects_b.push(obj)
	});
	...
	// console.log(objects_b)

	let results = {A:[], R:[]};

	let proc = (part) => {
		let in_wf = instruction_a["in"]
		...
		out: while(1)
		{
			...
			for(; i < in_wf.length - 1; i++)
			{
				...
				//console.log(id, part[id], test, value, pass)
				...
				if (pass)
				{
					...
					{
						results[pointer].push(part)
						return;
					}
					else
					{
						in_wf = instruction_a[pointer];
						i = -1;
						continue;
					}
				}
	...
	for (let i = 0; i < objects_b.length; i++)
	{
		proc(objects_b[i]);
	}
