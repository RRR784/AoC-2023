let data = `Game 1: 1 green, 6 red, 4 blue; 2 blue, 6 green, 7 red; 3 red, 4 blue, 6 green; 3 green; 3 blue, 2 green, 1 red
...
(data => console.log(data
	...
	.map((el0, p) => (p = [0, 0, 0],el0
		...
		.forEach(el1 => el1
			.forEach(el2 => [0,1,2]
				.forEach(el3 => p[el3] = +el2[0] > p[el3] && el2[1][0] === 'rgb'[el3] ? +el2[0] : p[el3]))), p
		.reduce((a, c) => a * c)))
	.reduce((a, c) => a + c)))(data);

(data => console.log(data.split('\n').map((el0, p) => (p = [0, 0, 0],el0.split(': ')[1].split('; ').map(el1 => el1.split(', ').map(el2 => el2.split(' '))).forEach(el1 => el1.forEach(el2 => [0,1,2].forEach(el3 => p[el3] = +el2[0] > p[el3] && el2[1][0] === 'rgb'[el3] ? +el2[0] : p[el3]))), p.reduce((a, c) => a * c))).reduce((a, c) => a + c)))(data);


((data,i)=>(i=['split','map','forEach','reduce'],console.log(data[i[0]]('\n')[i[1]]((e0,p)=>(p=[0,0,0],e0[i[0]](': ')[1][i[0]]('; ')[i[1]](e1=>e1[i[0]](', ')[i[1]](e2=>e2[i[0]](' ')))[i[2]](e1=>e1[i[2]](e2=>[0,1,2][i[2]](e3=>p[e3]=+e2[0]>p[e3]&e2[1][0]=='rgb'[e3]?+e2[0]:p[e3]))),p[i[3]]((a,c)=>a*c)))[i[3]]((a,c)=>a+c))))(data);


console.log(((data,i)=>(i=['split','map','forEach','reduce'],data
	[i[0]]('\n')[i[1]]((e0,p)=>(p=[0,0,0],e0[i[0]](': ')[1][i[0]]
	('; ')[i[1]](e1=>e1[i[0]](', ')[i[1]](e2=>e2[i[0]](' ')))[i[2
	]](e1=>e1[i[2]](e2=>[0,1,2][i[2]](e3=>p[e3]=+e2[0]>p[e3]&e2[1
	][0]=='rgb'[e3]?+e2[0]:p[e3]))),p[i[3]]((a,c)=>a*c)))[i[3]]((
	a,c)=>a+c)))(data));