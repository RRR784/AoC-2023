...
let go = () => 
{
	...
	for (let y = 0; y < h; y++)
	{
		...
		for (let x = 0; x < w; x++)
		{
			if (data[y][x] === '.')
				continue;
			if (data[y].charCodeAt(x) >= 48 && data[y].charCodeAt(x) <= 57)
				continue;
			symbols[y].push(+x);
			all_symbols.add(data[y][x]);
		}
	}
	// console.log(symbols);
	...
	for (let y = 0; y < h; y++)
	{
		...
		for (let x = 0; x < w; x++)
		{
			...
			if (data[y].charCodeAt(x) >= 48 && data[y].charCodeAt(x) <= 57)
			{
				while(x < w && data[y].charCodeAt(x) >= 48 && data[y].charCodeAt(x) <= 57)
				{
					x++;
					end_index = x;
				}
			...
			nums[y].push(+data[y].substring(start_index, end_index));
			ranges[y].push([+start_index, +end_index]);
		}
	}
	// console.log(nums);
	// console.log(ranges);
	...
	for (let y = 0; y < ranges.length; y++)
	{
		...
		{
			x0 = Math.max(0, ranges[y][x][0] - 1);
			x1 = Math.min(w - 1, ranges[y][x][1] + 1);
			out: for (let yy = y0; yy <= y1; yy++)
			{
				for (let xx = x0; xx < x1; xx++)
				{
					if(all_symbols.has(data[yy][xx]))
					{
						results.push(nums[y][x]);
						break out;
					}
				}
			}
	...
	let result = results.reduce((a, c) => a + c);
...
data = `.....613...................................439............498.........................438......617....343.............942...................
