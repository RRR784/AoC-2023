let go = () => {
	...
	let snippers = [
		  'bdvhsd', 'hsdzzp', 'zzpggz', 'ggzfkb',
		  'fkbpjl', 'pjlmdx', 'mdxnsk', 'nskrsg',
		  'rsgqsd', 'qsdgds', 'gdsnfj', 'bdvffd',
		  'ffdxbm', 'xbmzhp', 'zhprfg', 'rfgjks',
		  'jkstlq', 'tlqtmq', 'tmqhfx', 'hfxvlk',
		  'vlklkd', 'lkdnfj', 'bdvhzf', 'hzffgv',
		  'fgvpbh', 'pbhbqc', 'bqcznx', 'znxqjx',
		  'qjxlsg', 'lsgzjm', 'zjmzcp', 'zcpsnb',
		  'snbtht', 'thtdgk', 'dgkbpc', 'bpcfth',
		  'fthnfj'
		];
	...
	let travelled_nodes = new Set();

	let heatmap = {};
	Object.keys(map).forEach(el=>heatmap[el] = 0);
	let recursive_traverse = start => {
		map[start].forEach(el=>{
			heatmap[el]++;
			if(!travelled_nodes.has(el))
			{
				travelled_nodes.add(el)
				recursive_traverse(el)
			}
		});
	}
	recursive_traverse(keys[rand(keys.length)]);

	console.log(Object.keys(map).length, travelled_nodes.size)
	console.log(travelled_nodes.size, Object.keys(map).length - travelled_nodes.size)
	console.log(travelled_nodes.size * (Object.keys(map).length - travelled_nodes.size));
}
