...
let rand = size => Math.floor(Math.random() * size);
let canonical_name = (a, b) => [a, b].sort().join("");
let go = () => {
	data = data.split("\n").map(el=>el.split(": ").map(el2=>el2.split(" ")));
	let map = {};
	data.forEach(el=>{
		map[el[0][0]] = new Set(el[1]);
	})
	let copy_map = (input) => {
		let res = {};
	...
	// the incoming node needs to be checked
	// then it blindly adds it's neighbor nodes
	let do_traverse = (node, costs, src) => {
		let name = node.name;
		let cost = node.value;
		let prev_cost = costs[name];
		cost++;
		if (cost < prev_cost)
		{
			costs[name] = cost;
			let next_names = [...src[name]];
			for (let i = 0; i < next_names.length; i++)
			{
				heap_insert(get_node(next_names[i], cost));
			}
			return;
		}
	}
	...
	let get_path = (start, cost_map) => {
		let path = [start];
		while(1)
		{
			let next_names = [...map[start]];
			let min = ['', Infinity];
			for (let i = 0; i < next_names.length; i++)
			{
				let next_name = next_names[i];
				if (cost_map[next_name] < min[1])
				{
					min[0] = next_name;
					min[1] = cost_map[next_name];
				}
			}
			path.push(min[0])
			start = min[0];
	...
	let start = keys[rand(keys.length)]

	// get a fresh cost map

	let cost_map = copy_map(map);

	// insert the start node and calculate all paths

	heap_insert(get_node(start, -1), cost_map, map)

	while(bin_heap.length)
	{
		do_traverse(heap_extract(), cost_map, map)
	}

	// get the most expensive path

	let max_costs = Object.entries(cost_map).sort((a, b)=>(b[1]-a[1]));
	let end = max_costs[0][0];

	// get the connections and remove them

	let connections = get_path(end, cost_map)
	all_connections = [...all_connections, ...connections];
	connections.forEach(el=>{
		let a = el.substring(0, 3);
		let b = el.substring(3, 6);
		remove_pair(a, b);
	})
	...
	for(let i = 0; i < 2; i++)
	{
		cost_map = copy_map(map);
		heap_insert(get_node(start, -1), cost_map, map)
		while(bin_heap.length)
		{
			do_traverse(heap_extract(), cost_map, map)
		}
		connections = get_path(end, cost_map)
		all_connections = [...all_connections, ...connections];
		connections.forEach(el=>{
			let a = el.substring(0, 3);
			let b = el.substring(3, 6);
